...
def train_movie(movie: str, device:torch.device, results_folder: str, seqlen: int, train_batch_size: int, \
    eval_batch_size: int, encoder_learning_rate: float, learning_rate: float, max_epochs: int, max_norm: float, \
    ...
    train_loader, test_loader, dev_loader = get_dataloaders(results_folder, seqlen, train_batch_size, \
        eval_batch_size, movie, device)
    print("{:25s}: {:3d} train batches, {:3d} dev batches, {:3d} test batches".format(name, len(train_loader), \
        len(dev_loader), len(test_loader)))
...
def train(data_folder: str, results_folder: str, seqlen: int, train_batch_size: int, eval_batch_size: int, \
    ...
    max_epochs: int, patience: int, max_norm: float, parallel: bool, n_folds_per_gpu: int, verbose = False):
    ...
    arguments = [results_folder, seqlen, train_batch_size, eval_batch_size, encoder_learning_rate, learning_rate, \
        max_epochs, max_norm, verbose]
    test_perfs = []
